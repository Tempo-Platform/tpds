...
import { H1, P, H6, PSmall } from '../../../tpds/elements/typography'
...
const MainColorsRow = styled.div(() => [tw`block sm:(grid grid-cols-8 gap-2)`])
const ColorsRow = styled.div(() => [tw`block sm:(grid grid-cols-10 gap-2)`])
const ColorLabel = styled(PSmall)(() => [tw`mb-[2px]`])
const ColorHexLabel = styled(ColorLabel)(() => [tw`font-mono text-tertiary mb-2`])
const SwatchItem = styled.div(() => [tw`mb-2 grid grid-cols-2 gap-4 sm:(block mb-0)`])
const ColorSwatch = styled.div(() => [tw`aspect-auto rounded sm:aspect-video`])
