...
export default function Page() {
  ...
  const onThumbnail0Change = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
  ...
  const onThumbnail1Change = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      const updateImagesData = [...imagesData]
  ...
  const onThumbnail2Change = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      const updateImagesData = [...imagesData]
  ...
  const onThumbnail3Change = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      const updateImagesData = [...imagesData]
  ...
  const onThumbnail4Change = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      const updateImagesData = [...imagesData]
  ...
  const onThumbnail5Change = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      const updateImagesData = [...imagesData]
  ...
  return (
    <div>
      <Container>
        ...
        <br />
        <P>
          You can tweak the style using the controls on the right side. The code example below will change as you tweak
          the values.
        </P>
        <br />
        <P>
          In this demo, you can <span className='font-bold'>click the background image or the thumbnails</span> to
        ...
        <div className='flex flex-col gap-y-10 md:flex-row gap-x-12 items-start'>
          <div
            ...
            style={{
              ...
              minWidth: '320px',
              maxWidth: '400px',
              backgroundImage: `url(${backgroundImage ? backgroundImage : 'https://picsum.photos/300/520?c=2'})`,
          ...
          <Window className='w-full md:w-auto'>
            ...
            <div className='flex gap-x-2'>
              <Button variant={numImages === 2 ? 'info' : 'secondary'} onClick={() => setNumImages(2)}>
                2
              </Button>
              <Button variant={numImages === 3 ? 'info' : 'secondary'} onClick={() => setNumImages(3)}>
                3
              </Button>
              <Button variant={numImages === 4 ? 'info' : 'secondary'} onClick={() => setNumImages(4)}>
                4
              </Button>
              <Button variant={numImages === 5 ? 'info' : 'secondary'} onClick={() => setNumImages(5)}>
                5
              </Button>
              <Button variant={numImages === 6 ? 'info' : 'secondary'} onClick={() => setNumImages(6)}>
                6
              </Button>
            </div>
            ...
            <ColorPicker hexColor={backgroundColor} onChangeHandler={value => setBackgroundColor(value)} />
            <PSmall isMedium className='mb-1 mt-5 text-tertiary'>
