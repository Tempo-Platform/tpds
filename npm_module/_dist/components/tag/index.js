...
const Tag = ({
  ...
  const finalRootClass = clsx(props.className ? props.className : '', 'border inline-flex rounded py-0.5 px-1.5 select-none flex justify-center', addHoverEffect && 'cursor-pointer', addHoverEffect ? tagVariantsWithHover[variant] : tagRootVariantStyles[variant]);
  const finalLabelClass = clsx('text-[11px] font-bold', isUppercase && 'uppercase', addHoverEffect ? tagLabelVariantsWithHover[variant] : tagLabelVariantStyles[variant]);
  console.log('finalRootClass', finalRootClass);
  console.log('finalLabelClass', finalLabelClass);
  return <div className={finalRootClass}>
...
const defaultLabelStyleWithHover = [`text-grey-light-scale-600 hover:text-white hover:dark:text-black`];
