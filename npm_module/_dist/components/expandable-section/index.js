...
function ExpandableSection({
  ...
  const rootClassName = clsx(`bg-window border border-window`, removeBorderTop && `border-t-0`, removeBorderBottom && `border-b-0`);
  const headerClassName = clsx(`h-[44px] w-full flex justify-between items-center px-[16px] transition-all cursor-pointer`, isOpen ? `border-b border-window` : `border-b border-transparent`);
  ...
  return <>
      ...
      <div className={rootClassName} ref={headerRef}>
        <div className={headerClassName} onClick={() => handleToggleHeader(isOpen, toggleOpen, headerRef, excludeElementId)}>
