...
const ThemeToggle = ({
  ...
  return <button {...props} className='theme-toggle' tw="w-6 h-6 focus:outline-none fill-current text-primary hover:text-secondary" onClick={() => setTheme(isDark() ? 'light' : 'dark')}>
      {isDark() ? <Moon /> : <Sun />}
    </button>;
};
