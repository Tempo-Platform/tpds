...
export const Button = ({
  ...
  return (
    <ButtonElement variant={variant} className={className} onClick={onClick} {...props}>
      ...
      {label}
      {iconAfter && iconAfter}
      {loading && <Spinner className='block' radius={15} />}
    </ButtonElement>
  )
}
...
export function ExternalLinkIcon({ ...props }) {
  return (
    <svg {...props} xmlns='http://www.w3.org/2000/svg' width='14' height='14' viewBox='0 0 13 14'>
      <path
...
export function ExternalLinkButton({ label, variant, ...props }: { label: string; variant?: string }) {
  return <Button variant={variant || 'default'} label={label} iconAfter={<ExternalLinkIcon />} {...props} />
}
