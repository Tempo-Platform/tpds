...
import { Calendar } from '../../assets/svgs/icons'
...
function DatePicker({
  ...
  for (let i = 0; i < pickers.length; i++) {
    ...
    if (i !== pickers.length - 1) {
      renderItems.push(
        <div key={'division' + i} className='w-[4px] h-[2px] bg-black dark:bg-white relative opacity-30 top-[1px]' />,
      )
    }
  }
  ...
  return (
    <div className={rootClassName} style={styleOverrides}>
      {renderItems}
      {!omitCalendarIcon && <Calendar className='text-tertiary opacity-70 ml-2' />}
    </div>
  )
}
