...
export async function getDesignTokens(url = '', data = {}) {
    ...
    if (currentNodeType === 'COLOR') {
      ...
      for (var sh = 0; sh < currentNodeData.effects.length; sh++) {
        ...
        if (fx.type === 'DROP_SHADOW') {
          const newShadow = `${fx.offset.x}px ${fx.offset.y}px ${fx.radius}px ${fx.spread || 0}px rgba(${fx.color.r},${
            fx.color.g
          },${fx.color.b},${fx.color.a.toFixed(2)})`
          if (shadowValue !== '') {
  ...
  fs.writeFileSync(path.join(__dirname, '../src/tokens/index.js'), 'export default ' + util.inspect(finalDataObject, false, 5, false))
